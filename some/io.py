...
def write_jsonl(path: str, data: List[Any]) -> None:
    """Write data as JSONL (JSON Lines) format, creating parent dirs as needed."""
    dir_path = os.path.dirname(path)
    if dir_path:
        os.makedirs(dir_path, exist_ok=True)
    with open(path, "w", encoding="utf-8") as f:
...
def read_jsonl(path: str, default: Optional[List[Any]] = None) -> List[Any]:
    """Read JSONL file; return default if missing or invalid."""
    if os.path.exists(path):
        try:
            items = []
            with open(path, "r", encoding="utf-8") as f:
